...
function getDefaultActivePaths(tree) {
  const p = {};
  (function walk(n) { if (n.children.length > 0) { p[n.id] = n.children[0].id; n.children.forEach(walk); } })(tree);
  return p;
}
...
export default function StrataDesigner() {
  const [tree, setTree] = useState(initialTree);
  const [selectedId, setSelectedId] = useState("funds");
  const [activePaths, setActivePaths] = useState(() => getDefaultActivePaths(initialTree));
  ...
  const resetDescendantPaths = useCallback((nodeId, paths) => {
    ...
    // Priority: redirect target, then default child, then first child
    const defaultChildId = node.redirectChild || node.defaultChild || node.children[0].id;
  ...
  const handleAddChild = useCallback((parentId, name, param) => {
    const nc = { id: generateId(), name, param: param || null, slots: [], fills: [], children: [], defaultChild: null, redirectChild: null };
    ...
    <div style={{ minHeight: "100vh", background: "#0a0f1a", color: "#e2e8f0", fontFamily: sans }}>
      <div style={{
        ...
        <div style={{ maxWidth: 1400, margin: "0 auto", padding: "10px 16px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div style={{ display: "flex", gap: 2, background: "#1e293b", borderRadius: 7, padding: 2, flexShrink: 0, marginLeft: 12 }}>
